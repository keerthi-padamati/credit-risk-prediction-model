...
def data_cleaning(app: pd.DataFrame, credit: pd.DataFrame):
    ...
    app.drop(columns=['OCCUPATION_TYPE', 'FLAG_PHONE'], inplace=True, errors='ignore')
    print(f"  Dropped OCCUPATION_TYPE and FLAG_PHONE columns")
...
def logistic_model(xtrain, xtest, ytrain, ytest):
    ...
    _plot_confusion_matrix(confusion_matrix(ytest, y_pred), 'Logistic Regression')
...
def xgboost_model(xtrain, xtest, ytrain, ytest):
    print("\n[MODEL] XGBoost …")
    ratio = float(ytrain.value_counts()[0]) / ytrain.value_counts()[1]
    xgb = XGBClassifier(eval_metric='logloss', random_state=42, use_label_encoder=False, verbosity=0, scale_pos_weight=ratio)
